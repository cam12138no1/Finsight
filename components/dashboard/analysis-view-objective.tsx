    ...
    <div className="p-6">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        {analysis.one_line_conclusion && (
          <div className="p-6 bg-[#F9F9F6] rounded-xl border border-blue-100">
            <h3 className="text-sm font-semibold text-emerald-600 mb-2 flex items-center gap-2">
              <Target className="h-4 w-4" />
              核心数据摘要
            </h3>
            <p className="text-lg font-medium text-[#1F2937] leading-relaxed">
        ...
        {analysis.results_table && analysis.results_table.length > 0 && (
          <div>
            <h3 className="text-sm font-semibold text-[#6B7280] mb-4 flex items-center gap-2">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-[#F9F9F6]">
                    <th className="px-4 py-3 text-left font-semibold text-[#6B7280]">指标</th>
                    <th className="px-4 py-3 text-left font-semibold text-[#6B7280]">实际值</th>
                    {hasResearchReport && (
                      <th className="px-4 py-3 text-left font-semibold text-[#6B7280]">市场预期</th>
                    )}
                    <th className="px-4 py-3 text-left font-semibold text-[#6B7280]">同比变化</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#E8E8E3]">
                  {analysis.results_table.map((row, idx) => (
                    <tr key={idx} className="hover:bg-[#F9F9F6]">
                      <td className="px-4 py-3 font-medium text-[#1F2937]">{row.metric}</td>
                      <td className="px-4 py-3 text-[#4B5563] font-mono">{row.actual}</td>
                      {hasResearchReport && (
                        <td className="px-4 py-3 text-[#6B7280] font-mono">{row.consensus || '-'}</td>
                      )}
                      <td className="px-4 py-3">
                        <span className={`font-medium font-mono ${
                          row.delta?.startsWith('+') ? 'text-emerald-600' :
                          row.delta?.startsWith('-') ? 'text-red-600' : 'text-[#6B7280]'
                        }`}>
                          {row.delta || '-'}
                        </span>
                      </td>
        ...
        {analysis.results_summary && (
          <div className="p-4 bg-[#F9F9F6] rounded-xl border border-[#E8E8E3]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {analysis.drivers.demand && (
                <div className="p-4 bg-emerald-50/50 rounded-xl border border-blue-100">
                  <h4 className="font-semibold text-emerald-700 mb-2">{analysis.drivers.demand.title}</h4>
                  {analysis.drivers.demand.metrics && (
                    <p className="text-sm text-emerald-600 mb-2">{analysis.drivers.demand.metrics}</p>
                  )}
                  <p className="text-sm text-[#4B5563] mb-1"><strong>变化：</strong>{analysis.drivers.demand.change}</p>
                  <p className="text-sm text-[#4B5563] mb-1"><strong>幅度：</strong>{analysis.drivers.demand.magnitude}</p>
                  <p className="text-sm text-[#6B7280]"><strong>原因：</strong>{analysis.drivers.demand.reason}</p>
                </div>
              )}
              {analysis.drivers.monetization && (
                <div className="p-4 bg-emerald-50/50 rounded-xl border border-green-100">
                  <h4 className="font-semibold text-emerald-700 mb-2">{analysis.drivers.monetization.title}</h4>
                  {analysis.drivers.monetization.metrics && (
                    <p className="text-sm text-emerald-600 mb-2">{analysis.drivers.monetization.metrics}</p>
                  )}
                  <p className="text-sm text-[#4B5563] mb-1"><strong>变化：</strong>{analysis.drivers.monetization.change}</p>
                  <p className="text-sm text-[#4B5563] mb-1"><strong>幅度：</strong>{analysis.drivers.monetization.magnitude}</p>
                  <p className="text-sm text-[#6B7280]"><strong>原因：</strong>{analysis.drivers.monetization.reason}</p>
                </div>
              )}
              {analysis.drivers.efficiency && (
                <div className="p-4 bg-purple-50 rounded-xl border border-purple-100">
                  ...
                  <p className="text-sm text-[#4B5563] mb-1"><strong>变化：</strong>{analysis.drivers.efficiency.change}</p>
                  <p className="text-sm text-[#4B5563] mb-1"><strong>幅度：</strong>{analysis.drivers.efficiency.magnitude}</p>
                  <p className="text-sm text-[#6B7280]"><strong>原因：</strong>{analysis.drivers.efficiency.reason}</p>
                </div>
              )}
            </div>
          </div>
        ...
        {analysis.investment_roi && (
          <div className="p-6 bg-[#F9F9F6] rounded-xl border border-[#E8E8E3]">
            <h3 className="text-sm font-semibold text-[#6B7280] mb-4 flex items-center gap-2">
              <DollarSign className="h-4 w-4" />
              投入与资本支出数据
            </h3>
            <div className="space-y-3">
              <p className="text-sm text-[#4B5563]"><strong>CapEx变化：</strong>{analysis.investment_roi.capex_change}</p>
              <p className="text-sm text-[#4B5563]"><strong>Opex变化：</strong>{analysis.investment_roi.opex_change}</p>
              <p className="text-sm text-[#4B5563]"><strong>投入方向：</strong>{analysis.investment_roi.investment_direction}</p>
              {analysis.investment_roi.roi_evidence && analysis.investment_roi.roi_evidence.length > 0 && (
                <div>
                  <p className="text-sm font-medium text-[#4B5563] mb-1">ROI数据：</p>
              ...
              <p className="text-sm text-[#4B5563]"><strong>管理层表态：</strong>{analysis.investment_roi.management_commitment}</p>
            </div>
          </div>
        )}
              ...
              <div className="p-4 bg-emerald-50/50 rounded-xl border border-green-100">
                ...
                <ul className="space-y-2">
                  {analysis.sustainability_risks.sustainable_drivers.map((driver, idx) => (
                    <li key={idx} className="text-sm text-[#4B5563] flex items-start gap-2">
        ...
        {hasResearchReport && analysis.research_comparison && (
          <div className="p-6 bg-[#F9F9F6] rounded-xl border border-[#E8E8E3]">
            <h3 className="text-sm font-semibold text-[#6B7280] mb-4">研报数据对比</h3>
            ...
            {analysis.research_comparison.key_differences && analysis.research_comparison.key_differences.length > 0 && (
              <div>
                <p className="text-sm font-medium text-[#4B5563] mb-1">数据差异：</p>
                <ul className="list-disc list-inside text-sm text-[#6B7280] space-y-1">
